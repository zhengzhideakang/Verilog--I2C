...
module i2cMaster #(
  ...
  (* mark_debug *)output wire        fifo_rd_en,
  (* mark_debug *)input  wire        fifo_empty,

  //~ 读/写数据过程指示
  (* mark_debug *)output wire [6 :0] device_addr,     // 当前正在操作的从机设备地址
  (* mark_debug *)output wire [15:0] data_addr,       // 当前正在操作的从机数据地址
  (* mark_debug *)output wire [7 :0] wdata,           // 写入的数据
  ...
  output wire sda_o,
  (* mark_debug *)output wire sda_oen,
  (* mark_debug *)input  wire scl_i,
  output wire scl_o,
  (* mark_debug *)output wire scl_oen,
...
(* mark_debug *)wire rd_or_wr_n = fifo_dout[48]; // 读/写指示
...
(* mark_debug *)wire page_rd_en = ctrl_byte[7]; // 使能页读, 即连续读
(* mark_debug *)wire page_wr_en = ctrl_byte[6]; // 使能页写, 即连续写
/*
  从机数据地址字节数减1, 0表示8位数据地址, 1表示16/17/18位数据地址
  目前I2C器件最大存储空间为2Mbit(型号AT24CM02), 对应数据地址18位, 对应2**18Byte=2**21bit=2Mbit
*/
(* mark_debug *)wire this_slave_data_addr_bytes_minus_1 = ctrl_byte[5];
...
(* mark_debug *)reg [10:0] state;
(* mark_debug *)reg [10:0] next;
//~ 初始态与状态跳转
...
//++ 三段式状态机-状态跳转 ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
(* mark_debug *)wire start_end;          // 指示开始条件结束
(* mark_debug *)wire device_addr_wr_end; // 指示设备地址写结束
(* mark_debug *)wire wr_data_addr_end;   // 指示数据地址写结束
(* mark_debug *)wire wr_data_end;        // 指示数据写结束
(* mark_debug *)wire device_addr_rd_end; // 指示设备地址读结束
(* mark_debug *)wire rd_data_end;        // 指示数据读结束
(* mark_debug *)wire rd_data_ack_end;    // 指示数据读应答结束
(* mark_debug *)wire rd_data_no_ack_end; // 指示数据读不应答结束
(* mark_debug *)wire stop_end;           // 指示停止条件结束
(* mark_debug *)reg  receive_ack_no_goto_idle; // 未收到从机的应答, 认为通信中断, 转入IDLE
...
(* mark_debug *)reg [15:0] this_slave_data_addr; //~ 当前从机内部的数据地址
always @(posedge clk) begin
...
(* mark_debug *)wire goto_device_addr_rd = rd_or_wr_n
                                            && device_addr == this_slave_device_addr
                                            && data_addr ==  this_slave_data_addr;

(* mark_debug *)wire continue_wr_data = ~rd_or_wr_n
                                        && device_addr == this_slave_device_addr
                                        && data_addr == this_slave_data_addr
                                        && page_wr_en;

(* mark_debug *)wire continue_rd_data = goto_device_addr_rd && page_rd_en;
...
//++ 生成scl ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
(* mark_debug *)reg [15:0] scl_clk_cnt_max; // 控制I2C总线频率
always @(posedge clk) begin
  case (state)
...
(* mark_debug *)reg [15:0] scl_clk_cnt;
(* mark_debug *)wire i2c_slave_is_clock_stretching;
always @(posedge clk) begin
...
(* mark_debug *)reg [3:0] scl_cnt; // I2C的scl计数
reg scl_oen_reg;
always @(*) begin
  scl_oen_reg <= 1'b0;
  case (state)
...
(* mark_debug *)reg wr_data_addr_byte_cnt;
always @(posedge clk) begin
